import {PROJECT_LOGO,PROJECT_NAME, PROJECT_EMAIL_STATIC, PROJECT_NUMBER} from '../miscellaneous/constants';
...
<body style="font-family: Arial, sans-serif; margin: 20px; color: #333;">
    <div style="max-width: 800px; margin: 0 auto;">
        <div style="display: flex; align-items: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #eee;">
            <img src=${payload.agency_data?.logo || PROJECT_LOGO} alt="online travel agency" style="width: 50px; height: 50px; margin-right: 15px;">
            <div>
                <div style="font-size: 24px; font-weight: bold; color: #2c3e50;">${payload.agency_data?.agency_name || PROJECT_NAME}</div>
                <div style="font-size: 14px; color: #7f8c8d; margin-top: 5px;">
                    <div>Email: ${payload.agency_data?.agency_email || PROJECT_EMAIL_STATIC}</div>
                    <div>Phone: ${payload.agency_data?.agency_phone || PROJECT_NUMBER}</div>
                </div>
            </div>
        </div>
