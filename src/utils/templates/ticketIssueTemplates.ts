...
export interface ITicketIssueEmailTemplate {
...
export interface ITicketIssuePDFTemplate {
  agency?: {
    email: string;
    phone: string;
    address: string;
...
export const template_onTicketIssue = (payload: ITicketIssueEmailTemplate) => {
  return `
    <!DOCTYPE html>
    <html lang="en">
        <head>
                ...
                <div style="padding: 20px;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 14px;">
                        <tr>
                            ...
                            <td style="border: 1px solid #ddd; padding: 10px;">${payload.ticket_numbers?.join(
                              ", "
                            )}</td>
                        </tr>
                        <tr>
...
export const template_onTicketIssueReminder = (payload: ITicketIssueEmailTemplate & { last_issue_time?: string }) => {
  return `
    <!DOCTYPE html>
    <html lang="en">
        <head>
            ...
            <div style="max-width: 600px; margin: 20px auto; border: 1px solid #ddd; border-radius: 8px; background-color: #ffffff;">
                ...
                <div style="padding: 20px;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 14px;">
                        <tr>
                            <th style="border: 1px solid #ddd; padding: 10px; background-color: #f2f2f2; text-align: left;">Ticket Numbers</th>
                            <td style="border: 1px solid #ddd; padding: 10px;">${payload.ticket_numbers?.join(", ") || "N/A"}</td>
                        </tr>
                        <tr>
                            <th style="border: 1px solid #ddd; padding: 10px; background-color: #f2f2f2; text-align: left;">Journey Type</th>
                        ...
                        <tr>
                            <th style="border: 1px solid #ddd; padding: 10px; background-color: #f2f2f2; text-align: left;">Total Price</th>
                            <td style="border: 1px solid #ddd; padding: 10px;">${payload.payable_amount ?? "N/A"} BDT</td>
                        </tr>
                        <tr>
                            <th style="border: 1px solid #ddd; padding: 10px; background-color: #f2f2f2; text-align: left;">Due Amount</th>
                            <td style="border: 1px solid #ddd; padding: 10px;">${payload.due_amount ?? "N/A"} BDT</td>
                        </tr>
                        ${
...
export const template_onTicketInProcess = (payload: ITicketIssueEmailTemplate) => {
  return `
    <!DOCTYPE html>
    <html lang="en">
        <head>
...
export const template_pdf_flightBookingDetails = (
  payload: ITicketIssuePDFTemplate
) => {
  const {
    bookingDate,
    bookingId,
  ...
  const travelSegmentsRows = travelSegments
    .map(
      ...
      <tr>
        ...
        <td style="padding: 10px; border: 1px solid #ddd;">${segment.departureDate.toDateString()}, ${
        segment.departureTime.split("+")[0]
      }</td>
        <td style="padding: 10px; border: 1px solid #ddd;">${segment.arrivalDate.toDateString()}, ${
        segment.arrivalTime.split("+")[0]
      }</td>
      </tr>
    `
    )
...
export const template_onTicketStatusChange = (
  payload: Omit<ITicketIssueEmailTemplate, "travel_date"> & { status: string }
) => {
            ...
            <div style="max-width: 600px; margin: 20px auto; border: 1px solid #ddd; border-radius: 8px; background-color: #ffffff;">
                ...
                <div style="padding: 20px;">
                    ...
                    <p style="font-size: 16px; color: ${
                      [FLIGHT_TICKET_ISSUE, FLIGHT_BOOKING_REFUNDED].includes(
                        status
                      )
                        ? "green"
                        : "red"
                    };">This booking has been ${status}.</p>
                </div>
            </div>
        </body>
    </html>
