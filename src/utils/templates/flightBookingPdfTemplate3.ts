  ...
  return `
      <div style="font-family: Arial, sans-serif; margin: 20px; color: #333;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            ...
            ${payload.passengerInfo
              .map(
                (p) => `
              <tr>
                <td>${p.first_name + " " + p.last_name}</td><td>${
                  p.type
                }</td><td>${p.date_of_birth}</td><td>${
                  p.passport_number || ""
                }</td><td>${p.passport_expiry_date || ""}</td>
              </tr>
            `
              )
              .join("")}
        ...
        <div style="margin-bottom: 20px;">
          ...
          <div>${payload.flightDetails.origin} → ${
    payload.flightDetails.destination
  }</div>
          <div><img src="${
            payload.flightDetails.airline_logo
          }" alt="Project Logo" style="width: 100%; height: 100%; object-fit: contain;"> ${
    payload.flightDetails.airline
  } | Flight No - ${payload.flightDetails.flight_number} | Aircraft Model - ${
    payload.flightDetails.aircraft
  }</div>
          <table style="width: 100%; border-collapse: collapse;">
            ...
            ${payload.flightDetails.segments
              .map(
                (s) => `
              <tr>
                <td>${s.date}</td><td>${s.time}</td><td>${
                  s.flightInfo
                }</td><td>${s.flightTime || ""}</td><td>${s.cabin}</td><td>${
                  s.baggage
                }</td>
              </tr>
            `
        ...
        <div style="margin-bottom: 20px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            ...
            <tr>
              <td>${payload.fareDetails.baseFare}</td><td>${
    payload.fareDetails.tax
  }</td><td>${payload.fareDetails.aitVat}</td><td>${
    payload.fareDetails.commission
  }</td><td>${payload.fareDetails.person}</td><td>${
    payload.fareDetails.total
  }</td>
            </tr>
          </table>
        </div>
