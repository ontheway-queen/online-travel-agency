...
import { ManualBankTransferService } from '../services/manualBankTransfer.service';
import { ManualBankTransferValidator } from '../utils/validators/manualBankTransfer.validator';
...
export class ManualBankTransferController extends AbstractController {
  ...
  public createManualBankTransfer = this.asyncWrapper.wrap(
    {
      bodySchema: this.validator.createManualBankTransferSchema,
    },
    async (req: Request, res: Response) => {
      const { code, ...rest } = await this.services.createManualBankTransfer(
        req
      );
