...
class TourPackageBookingBTOCService extends AbstractServices {
  ...
  public async createTourPackageBooking(req: Request) {
    return this.db.transaction(async (trx) => {
      ...
      const data = await this.Model.tourPackageModel(trx).getSingleTourPackage(
        req.body.tour_id
      );
      if (!data.length) {
        return {
      ...
      const last_entry = await this.Model.lastServiceEntryModel(
        trx
      ).getLastRefId({ type: INVOICE_TYPE_TOUR });
      const booking_ref_id = `${PROJECT_CODE}-T-${(Number(last_entry) + 1)
        .toString()
        .padStart(5, "0")}`;
      rest.booking_ref = booking_ref_id;
      await this.Model.lastServiceEntryModel(trx).incrementLastRefId({
        type: INVOICE_TYPE_TOUR,
      });

      const tour_package_info = await model.insertTourPackageBook(rest);

      booking_info["booking_id"] = tour_package_info[0].id;

      await model.insertTourPackageBookContact(booking_info);
      ...
      await adminNotificationSubService.insertNotification({
        ...
        ref_id: tour_package_info[0].id,
        type: NOTIFICATION_TYPE_B2C_TOUR_BOOKING,
      });
  ...
  public async getSingleBookingInfo(req: Request) {
    ...
    const data = await model.getSingleBookingInfo(booking_id, id);
    if (!data) {
      return {
        success: false,
        code: this.StatusCode.HTTP_NOT_FOUND,
...
export default TourPackageBookingBTOCService;
