...
class TourPackageBTOCService extends AbstractServices {
  ...
  public async tourPackageList(req: Request) {
    ...
    const data = await model.getTourPackageListV2({ ...query, s_type: 'b2c' });
    // console.log(data)
    // console.log(query);
  ...
  //get country name
  public async getCountryName(req:Request){
    const model = this.Model.tourPackageModel();
    const country = await model.getCountryName();
...
export default TourPackageBTOCService;
