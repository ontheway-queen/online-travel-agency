...
export class AdminBtoBBookingService extends AbstractServices {
  constructor() {
    super();
  }
  //get list
  ...
  public async createMessage(req: Request) {
    return await this.db.transaction(async (trx) => {
      ...
      const agency = await this.Model.agencyModel(trx).getSingleAgency(
        support_data[0].agency_id
      );
  ...
  public async closeSupport(req: Request) {
    ...
    return this.db.transaction(async (trx) => {
      ...
      await Lib.sendEmail(
        [PROJECT_EMAIL_OTHERS_1],
        `B2B Booking support has been closed`,
