...
export interface IAdminAgentFlightManualBookingPayload {
    agency_id: number;
    ...
    flights: IAdminAgentFlightManualBookingFlightPayload[];
    travelers: IAdminAgentFlightManualBookingTravelerPayload[];
    status: typeof FLIGHT_BOOKING_CONFIRMED | typeof FLIGHT_TICKET_ISSUE;
    payment: 'full' | 'partial' | 'paid';
    partial_payment_rules?: {
        payment_percentage: number;
        payment_last_day: Date;
    }
}

interface IAdminAgentFlightManualBookingFlightPayload {
    airline_code: string;
...
interface IAdminAgentFlightManualBookingTravelerPayload {
    key: string;
