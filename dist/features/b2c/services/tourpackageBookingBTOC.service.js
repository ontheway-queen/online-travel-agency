...
class TourPackageBookingBTOCService extends abstract_service_1.default {
    ...
    createTourPackageBooking(req) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.db.transaction((trx) => __awaiter(this, void 0, void 0, function* () {
                ...
                const data = yield this.Model.tourPackageModel(trx).getSingleTourPackage(req.body.tour_id);
                if (!data.length) {
                    return {
                        success: false,
                ...
                rest["adult_price"] = data[0].b2c_adult_price;
                rest["child_price"] = data[0].b2c_child_price;
                rest["discount"] = data[0].b2c_discount;
                rest["discount_type"] = data[0].b2c_discount_type;
                ...
                const booking_ref_id = `${constants_1.PROJECT_CODE}-T-${(Number(last_entry) + 1)
                    .toString()
                    .padStart(5, "0")}`;
                rest.booking_ref = booking_ref_id;
                yield this.Model.lastServiceEntryModel(trx).incrementLastRefId({
                    type: constants_1.INVOICE_TYPE_TOUR,
                });
                const tour_package_info = yield model.insertTourPackageBook(rest);
                booking_info["booking_id"] = tour_package_info[0].id;
                yield model.insertTourPackageBookContact(booking_info);
                //create invoice
                ...
                yield adminNotificationSubService.insertNotification({
                    message: `A new tour booking from B2C. Booking id ${tour_package_info[0].id}`,
                    ref_id: tour_package_info[0].id,
                    type: constants_1.NOTIFICATION_TYPE_B2C_TOUR_BOOKING,
                });
                return {
                    success: true,
    ...
    getMyBookingHistory(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.user;
            const { from_travel_date, to_travel_date, title, user_name, status, user_id = id, limit, skip, } = req.query;
    ...
    getSingleBookingInfo(req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const model = this.Model.tourPackageBookingModel();
            const data = yield model.getSingleBookingInfo(booking_id, id);
            if (!data) {
                return {
                    success: false,
                    code: this.StatusCode.HTTP_NOT_FOUND,
...
exports.default = TourPackageBookingBTOCService;
